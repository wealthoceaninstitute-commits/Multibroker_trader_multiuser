// TradeForm.jsx — keep old UI/layout, improve internals only
'use client';
...
const normalizeClient = (c) => ({
  ...
  name: c?.name ?? c?.display_name ?? c?.client_name ?? c?.client_id ?? c?.userid ?? '',
  broker: c?.broker ?? '',
  ...c,
});
...
export default function TradeForm() {
  ...
  useEffect(() => {
    const userid = getUserid?.() || detectUserId();
    if (!userid || settingsAppliedRef.current) return;
    ...
    api.get('/user_settings', {
      ...
      .then((res) => {
        const td = res?.data?.trade_defaults;
        if (!td) return;

        settingsAppliedRef.current = true;

        if (!saved?.action && td.action) setAction(String(td.action).toLowerCase());
        if (!saved?.productType && td.product_type) setProductType(String(td.product_type).toUpperCase());
        if (!saved?.orderType && td.order_type) setOrderType(String(td.order_type).toUpperCase());
        if (!saved?.timeForce && td.order_duration) setTimeForce(String(td.order_duration).toUpperCase());
      })
      .catch(() => {});
  }, [saved]);
  ...
  useEffect(() => {
    ...
    const loadClients = async () => {
      const attempts = [
        () => api.get('/clients', { params, headers }),
        () => api.get('/clients', { params: { user_id: userid }, headers }),
        () => api.get('/get_clients', { params, headers }),
        () => api.get('/get_clients', { params: { user_id: userid }, headers }),
      ];
      ...
      for (const fn of attempts) {
        try {
          ...
          setClients((list || []).map(normalizeClient).filter((x) => x.client_id));
          return;
        } catch {
          // try next
        }
    ...
    const loadGroups = async () => {
      const attempts = [
        () => api.get('/groups', { params, headers }),
        () => api.get('/groups', { params: { user_id: userid }, headers }),
      ];

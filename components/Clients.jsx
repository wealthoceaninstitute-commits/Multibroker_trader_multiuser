...
const DEFAULT_RENDER_API = "";
import { API_BASE } from "../src/lib/apiBase";
import { authHeaders, getUserid } from "../src/lib/auth";
...
export default function Clients() {
    ...
    for (const url of urls) {
      try {
        ...
        const norm = Array.isArray(arr)
          ? arr.map((c) => ({ ...c, broker: (c?.broker || "motilal").toLowerCase() }))
          : [];
        if (norm.length > 0) {
          setClients(norm);
          return;
  ...
  async function loadGroups() {
    ...
    try {
      ...
      if (r.ok) {
        ...
        setGroups(arr);
        try { localStorage.setItem(LS_KEY_GROUPS, JSON.stringify(arr)); } catch {}
        return;
      }
  ...
  const openAdd = () => {
    setEditMode(false);
    setAddForm({ name:'', userid:'', password:'', pan:'', apikey:'', totpkey:'', capital:'' });
